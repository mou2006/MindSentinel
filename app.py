...
# =========================================================
# INITIALIZE / MIGRATE DISTRESS DATABASE
# =========================================================
...
def init_distress_db():
    ...
    columns = conn.execute(
        "PRAGMA table_info(distress_records)"
    ).fetchall()
    ...
    if "assessment_score" not in column_names:

        conn.execute("""
            ALTER TABLE distress_records
            ADD COLUMN assessment_score REAL DEFAULT 0
        """)
    ...
    if "mood" not in column_names:
        ...
        conn.execute("""
            ALTER TABLE distress_records
            ADD COLUMN mood TEXT DEFAULT 'Neutral'
        """)
    ...
    if "distress_score" not in column_names:

        conn.execute("""
            ALTER TABLE distress_records
            ADD COLUMN distress_score REAL DEFAULT 0
        """)
    ...
    if "early_warning" not in column_names:

        conn.execute("""
            ALTER TABLE distress_records
            ADD COLUMN early_warning TEXT DEFAULT ''
        """)
...
def mood():
    ...
    if request.method == "POST":
        ...
        flash(
            f"Your mood '{selected_mood}' has been recorded.",
            "success"
        )
